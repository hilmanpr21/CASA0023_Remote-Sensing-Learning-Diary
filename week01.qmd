...
-   Such as:
    -   sentinel
    -   landsat ![Passive Sensor](week_01_files/mediabag/Passive%20sensor.png)
...
-   Such as:
    ...
    ![Active Sensor](week_01_files/mediabag/active%20sensor.png)
...
![Electromagnetive Wave](week_01_files/mediabag/electromagnetive%20wave.png)
...
-   Electromagnetic waves are scatters by particles on the atmosphere ![Electromagnetive Wave](week_01_files/mediabag/wavelength%20reflection.png)

-   Smaller wavelengths scatter easier

-   Blue has the smallest wavelength ![RGB Wavelength](week_01_files/mediabag/wavelength.png)
...
![Raster Grid](week_01_files/mediabag/raster%20image.png)
...
![pixel band system](week_01_files/mediabag/pixel%20band%20system.png){width="70%"}
...
![band list](week_01_files/mediabag/band%20list.png) - We can take values for each wavelength (or a band of several wavelengths) across the electromagnetic spectrum to create a **spectral signature**. different feature on earth has different spectral signature, it will be used to identify different object. **spectral signature** can be **discrete** (eg. multispectral) or **continuous** (eg. hyperspectral) ![Spectral Signature](week_01_files/mediabag/spectral.png) - This spectral signature are used to differentiate object in the image. In earth observation we use this for identifying land cover in earth surface using **Normalized Difference Vegetation Index (NDVI), Normalized Difference Building Index (NDBI)** - for example in this week practical we identify the landcover by making scatter plot with band 4 (red, vegetation absorb) and band 8 (Near-Infrared, NIR, that vegetation strongly reflect). high values of NIR and low values of red represents dense vegetation. Meanwhile low values of red and NIR represent dense vegetation ![Scatter Plot band 4 and band 8](week_01_files/mediabag/scatter%20plot.png)

## Application

The increasing satellite remote sensing data on earth observation in the last two decades, allows new approaches on understanding urban setting [@millerCitiesSpacePotential2003]. Satellite remote sensing data offers some advantages, such as: - broader spatial coverage, - ability for routine update, - consistent measurement
...
Different from passive sensor, Synthetic Aperture Radar (SAR) is an active sensor that can see through cloud and the backscatter radar wave can determine physical properties such as density and surface texture [@hendersonSARApplicationsHuman1997]. The ability of SAR to detect horizontal and vertical structure allows to detect settlement based on texture, materials geometry and density. The result of settlement detection can be used for estimating city population by multiplying with number of occupants per dwelling. Harris [-@harrisSIRAImageryTunisia1985] used this methods for estimating population in Tunisia.

This methods is beneficial for developing countries because they have more informal types of dwellings. Radar Data is really helpful because population census happens only once in 10 years [@hendersonSARApplicationsHuman1997] which could not keep up the rapid urban growth.

### Passive Sensor

#### Evaluating Poverty using Nighttime Light (NTL) Satellite

Nighttime Light Satellite is a passive sensor launched by nasa to collect data of night light emission [@earthsciencedatasystemsNighttimeLights2021]. The data from this satellite provide unique perspective that can be beneficial for detecting global conflict, human night behaviour, ecological impact of artificial night light and so on.

One of the benefit of nighttime light data is to track socioeconomic inequality and poverty. Yu [-@yuPovertyEvaluationUsing2015] evaluated poverty rate in 2856 counties in china. The research compare the average light index from Visible Infrared day-night band with integrated poverty index. It used linear regression and comparison of class to check how accurate the remote sensing data to evaluate poverty. The result shows that day-night band data can be useful to evaluate poverty in china
...
-   Although, using remote sensing data looks more fancy, in some cases and context we can get the similar data that are more efficient and cost effective from governmental source [@millerCitiesSpacePotential2003]. For example we can use building permission data rather than satellite data to identify land use.
