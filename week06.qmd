...
Detail steps of clustering:
1. Give data
2. Then set series of rules, such as:
	- the radius in spectral feature to define where new cluster started
	- spectral distance measure
	- number of pixels to be considered before merging
	- max number of cluster
3. It will repeat until no pixel alocated again

![K-mean Clustering](week_06_files/k-mean clustering.png){width="100%"}

source: citation **Evolutionary Computation Theory for Remote Sensing Image Clustering: A Survey**



Other methods of unsuervised learning is **ISODATA**. This methods is specifically for earth observation data. This methods is similar to K-means clustering but with some additional rules, which are:
...
Generally **steps of supervised learning** are:
1. Class Definition
2. Pre-processing
3. Training
4. Pixel Assignment
5. Accuracy assignment
...
1. **Parametric**
	...
	- Methods in parametric:
		-  Maximum likelihood
1. **Non-parametric**
	- Non-parametric use data that are not normally distributed
	- Methods in parametric:
		- Density slicing
		- Parallelpiped
...
![Hard and Fuzzy Classification](week_06_files/fuzzy vs hard.png){width="100%"}
...
![pixel content](week_06_files/sub pixel.png){width="80%"}
...
![(right) Pixel (left) Object](week_06_files/pixel vs object.png){width="100%"}
...
![Trade off accuracy and interpretability](week_06_files/trade off accuracy and interpretability.png){width="100%"}
...
![Flood vulnerable map](week_06_files/flood vulnerable map.png){width="100%"}
...
![Drought vulnerable map example](week_06_files/Drought.png){width="100%"}

source: Youssef[-@ghasempourDroughtVulnerabilityAssessment2022] 
